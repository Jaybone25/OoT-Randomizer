...
class World:
    def __init__(self, world_id: int, settings: Settings, resolve_randomized_settings: bool = True) -> None:
        ...
        for hint_type, data in misc_dual_hint_table.items():
            ...
            item_locations = item_location.split(', ')
            print(item_locations)
            for item in item_locations:
                self.misc_dual_hint: dict[str, str] = {hint_type: self.hint_dist_user.get('misc_dual_hint', {}).get(hint_type, item) for hint_type, data in misc_dual_hint_table.items()}
                items.append(self.misc_dual_hint[hint_type])
                #print(self.misc_dual_hint[hint_type])
                print(items)
